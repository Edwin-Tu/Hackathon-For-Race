...
interface VoiceTurnPayload {
  transcript?: string;
  input_language?: string | null;
  reply_language: string;
  translated_reply?: string | null;
  agent: AgentReply;
  speech_delivery?: SpeechDelivery | null;
}
...
export default function ElderVoice() {
  ...
  // 真實雙語語音互動（UI 風格維持原設計）
  const [voiceLanguage, setVoiceLanguage] = useState<'zh-TW' | 'nan-TW'>('zh-TW');
  const [isRecording, setIsRecording] = useState(false);
  const [voiceBusy, setVoiceBusy] = useState(false);
  const [voiceResult, setVoiceResult] = useState<VoiceTurnPayload | null>(null);
  ...
  useEffect(() => {
    ...
    setVoiceLanguage(storedLanguage === 'nan-TW' ? 'nan-TW' : 'zh-TW');
    if (!localStorage.getItem('voiceSessionId')) {
      localStorage.setItem(
        'voiceSessionId',
        globalThis.crypto?.randomUUID?.() || `voice-${Date.now()}`,
      );
    }
  }, []);
  ...
  const speakMandarinFallback = (text: string) => {
  ...
  const applySpeechResult = (payload: VoiceTurnPayload) => {
    const audio = payload.speech_delivery?.audio_base64;
    const contentType = payload.speech_delivery?.content_type || 'audio/wav';
    if (audio) {
      const source = `data:${contentType};base64,${audio}`;
      setAudioSrc(source);
      const player = new Audio(source);
      void player.play().catch(() => {
        setToast('語音已準備完成，可按「播放回覆」重播');
      });
      return;
    }

    setAudioSrc('');
    // Browser voices generally support Taiwanese Mandarin, not Min Nan.
    // If the dedicated Taiwanese TTS is unavailable, speak the original
    // Agent reply in Mandarin rather than mispronouncing the translation.
    speakMandarinFallback(payload.agent.reply);
  };
  ...
  const submitVoice = async (audioBlob: Blob) => {
    ...
    try {
      ...
      form.append('audio', audioBlob, `resident-voice.${extension}`);
      form.append('language', voiceLanguage);
      form.append(
        'session_id',
        localStorage.getItem('voiceSessionId') || `voice-${Date.now()}`,
      );

      const response = await fetch('/api/smart-care/voice-turn', {
        method: 'POST',
        body: form,
      });
      const payload = (await response.json()) as VoiceTurnPayload & { detail?: string };
      if (!response.ok) throw new Error(payload.detail || '語音處理失敗');

      setVoiceResult(payload);
      applySpeechResult(payload);
    } catch (error) {
      setVoiceError(error instanceof Error ? error.message : '語音處理失敗');
    } finally {
      setVoiceBusy(false);
    }
  };
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onerror = () => {
        stopMediaTracks();
        setIsRecording(false);
        setVoiceError('錄音失敗，請重新允許麥克風權限');
      };
      recorder.start();
      setIsRecording(true);
    } catch (error) {
      stopMediaTracks();
      setVoiceError(error instanceof Error ? error.message : '無法開啟麥克風');
    }
  };
  ...
  const confirmVoiceAction = async (decision: 'confirm' | 'cancel') => {
    const token = voiceResult?.agent.confirmation_token;
    if (!token || !voiceResult?.agent.session_id) return;
    setVoiceBusy(true);
    setVoiceError('');
    try {
      const response = await fetch('/api/smart-care/voice-confirm', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: voiceResult.agent.session_id,
          confirmation_token: token,
          decision,
          language: voiceResult.input_language || voiceLanguage,
        }),
      });
      const confirmed = (await response.json()) as VoiceTurnPayload & { detail?: string };
      if (!response.ok) throw new Error(confirmed.detail || '確認操作失敗');
      const merged: VoiceTurnPayload = {
        ...voiceResult,
        ...confirmed,
        transcript: voiceResult.transcript,
      };
      setVoiceResult(merged);
      applySpeechResult(merged);
    } catch (error) {
      setVoiceError(error instanceof Error ? error.message : '確認操作失敗');
    } finally {
      setVoiceBusy(false);
    }
  };
  ...
  const replayVoice = () => {
    ...
    if (voiceResult?.agent.reply) speakMandarinFallback(voiceResult.agent.reply);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 雙語語音互動：沿用原本的大字卡片與按鈕視覺 */}
        <Box
          sx={{
            mt: view.type === 'blank' ? 'auto' : 3,
            borderRadius: '24px',
            border: '2px solid #dbeafe',
            bgcolor: '#fff',
            boxShadow: '0 6px 28px rgba(30,64,175,0.10)',
            p: 2.5,
            textAlign: 'center',
          }}
        >
          <Typography sx={{ fontSize: '1.15rem', fontWeight: 800, color: '#475569' }}>
            {voiceLanguage === 'nan-TW' ? '台語模式' : '中文模式'}
          </Typography>

          {voiceResult && (
            <Box sx={{ mt: 2, textAlign: 'left', bgcolor: '#f8fafc', borderRadius: 3, p: 2 }}>
              {voiceResult.transcript && (
                <Typography sx={{ fontSize: '1.1rem', color: '#64748b', mb: 1 }}>
                  我說：{voiceResult.transcript}
                </Typography>
              )}
              <Typography sx={{ fontSize: '1.35rem', fontWeight: 900, color: '#1e293b' }}>
                {voiceResult.translated_reply || voiceResult.agent.reply}
              </Typography>
              {voiceResult.translated_reply && (
                <Typography sx={{ fontSize: '0.95rem', color: '#94a3b8', mt: 1 }}>
                  Agent 原文：{voiceResult.agent.reply}
                </Typography>
              )}
              <Button
                onClick={replayVoice}
                sx={{ mt: 1.5, fontSize: '1rem', fontWeight: 800, borderRadius: 2 }}
              >
                🔊 播放回覆
              </Button>

              {voiceResult.agent.requires_confirmation && voiceResult.agent.confirmation_token && (
                <Box sx={{ mt: 2 }}>
                  <Typography sx={{ fontSize: '1.05rem', fontWeight: 800, color: '#92400e', mb: 1 }}>
                    {voiceResult.agent.confirmation_summary || '此操作需要您的確認'}
                  </Typography>
                  <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
                    <Button
                      ...
                      color="success"
                      disabled={voiceBusy}
                      onClick={() => void confirmVoiceAction('confirm')}
                      sx={{ py: 1.4, fontSize: '1.15rem', fontWeight: 900, borderRadius: 3 }}
                    >
                      ⭕ 確認
                    </Button>
                    <Button
                      ...
                      color="error"
                      disabled={voiceBusy}
                      onClick={() => void confirmVoiceAction('cancel')}
                      sx={{ py: 1.4, fontSize: '1.15rem', fontWeight: 900, borderRadius: 3 }}
                    >
                      ❌ 取消
                    </Button>
          ...
          <Button
            ...
            sx={{
              ...
              fontSize: '1.7rem',
              fontWeight: 900,
              '&:hover': { bgcolor: isRecording ? '#b91c1c' : '#1d4ed8' },
            }}
          >
            {voiceBusy
              ? '處理中，請稍候…'
          ...
          <Typography sx={{ mt: 1, fontSize: '0.95rem', color: '#64748b' }}>
            {isRecording ? '正在錄音，說完後請按停止' : '可在帳號設置切換中文或台語'}
          </Typography>
        </Box>
      </Box>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
              <Button
                ...
                sx={{
                  borderRadius: 2,
                  py: 1.4,
                  border: `3px solid ${voiceLanguage === 'zh-TW' ? '#2563eb' : '#e2e8f0'}`,
                  bgcolor: voiceLanguage === 'zh-TW' ? '#eff6ff' : '#fff',
                  color: voiceLanguage === 'zh-TW' ? '#1d4ed8' : '#64748b',
                  fontSize: '1.15rem',
                  fontWeight: 900,
                }}
              >
              ...
              <Button
                onClick={() => setVoiceLanguage('nan-TW')}
                sx={{
                  borderRadius: 2,
                  py: 1.4,
                  border: `3px solid ${voiceLanguage === 'nan-TW' ? '#2563eb' : '#e2e8f0'}`,
                  bgcolor: voiceLanguage === 'nan-TW' ? '#eff6ff' : '#fff',
                  color: voiceLanguage === 'nan-TW' ? '#1d4ed8' : '#64748b',
                  fontSize: '1.15rem',
                  fontWeight: 900,
                }}
              >
