...
_EXPLICIT_BOUNDARY = _rx(r"(?:不要執行|勿執行|無須執行|不是要執行|do not follow|應偵測|設定最大|僅做翻譯|只回傳分類結果|標記為不可信|只是測試|不含真實|placeholder|佔位符|請勿產生真實|不要提供真實|只做分類|只做分析|安全回覆|不讓.*執行|如何(?:偵測|防止|避免|測試|管理|輪替|驗證)|應如何|不應放|不要放真實|優先級概念|風險與緩解|是否含.{0,30}意圖.{0,30}不要回覆)")
