...
# AWS App Runner is closed to new customers. Keep this only for AWS accounts
# that were already App Runner customers before the availability cutoff.
if [[ "${ALLOW_LEGACY_APPRUNNER:-false}" != "true" ]]; then
  echo "App Runner deployment is legacy-only. Use scripts/cloud/deploy.sh (ECS Fargate)." >&2
  echo "For an existing App Runner account, set ALLOW_LEGACY_APPRUNNER=true explicitly." >&2
  exit 2
fi

REGION="${AWS_REGION:-us-west-2}"
SERVICE_NAME="${SERVICE_NAME:-smart-care-agent}"
STACK_NAME="${STACK_NAME:-smart-care-agent-apprunner}"
...
required=(VPC_ID PRIVATE_SUBNET_IDS RDS_SECURITY_GROUP_ID DEMO_USER_ID DEMO_PERSONA_ID DATABASE_URL API_BEARER_TOKEN)
...
aws cloudformation deploy \
  ...
  --template-file infra/apprunner/stack.yaml \
...
echo "Run: BASE_URL=${SERVICE_URL} API_BEARER_TOKEN='***' python3 scripts/cloud/smoke_cloud.py"
